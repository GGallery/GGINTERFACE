        ...
        if (sizeof($items) > 0) {
            ...
            foreach ($items as $item) {
                if (isset($item->titolo)) {
                    ...
                    $out.='<a class="link' . $activeclass . '" href="' . JURI::base() . "component/gginterface/unita/" . $item->alias . '">' . $item->titolo . $badge . '</span></a>';
...
//                $sub_content = gginterfaceHelper::getTOTContenuti($item->id);
//                $sub_unit = gginterfaceHelper::getSubUnit($item->id);
...
//                    unset($res[$key]);
    ...
    public static function DISATTIVATOgetContentIconStatus($prerequisiti, $stato) {
        ...
        if (!$prerequisiti) {
            echo '<img class="img-rounded" title="Contenuto non ancora visionabile" src="components/com_gginterface/images/state_red.jpg"> ';
        } else {
            if ($stato == "completed") {
                echo '<img class="img-rounded" title="Contenuto già visionato" src="components/com_gginterface/images/state_green.jpg">';
            } else {
                echo '<img class="img-rounded" title="Contenuto da visionare" src="components/com_gginterface/images/state_grey.jpg"> ';
            }
        }
    }
    ...
    public static function DISATTIVATOgetContent_Footer($item){
        ...
        echo '<a href="component/gginterface/contenuto/'. $item['alias'] . '"  title="'.htmlentities(utf8_decode($item['abstract'])).'" >';
        ?>
        <div class="boxContentFooter img-rounded">
            ...
            <div class="boximg">
                ...
                if(file_exists('../mediagg/contenuti/'.$item["id"].'/'.$item["id"].'.jpg'))
                    ...
                    echo '<img class="img-responsive" src="components/com_gginterface/images/sample.jpg">';
                ?>
            </div>
